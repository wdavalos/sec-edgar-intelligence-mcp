...
import { resolveCIK, getCompanySubmissions, extractTicker, getXBRLFacts, buildFilingList, buildMaterialEvents, buildInsiderTrades, get10KContent, } from './sec-api.js';
const TOOL_PRICES = {
...
async function chargePPE(toolName) {
...
export async function searchCompanyFilings(params) {
...
export async function get10KAnnualReport(params) {
    ...
    for (let i = 0; i < recent.form.length; i++) {
        ...
        if (params.year) {
            ...
            if (parseInt(filingYear) !== params.year)
                continue;
        }
        targetIdx = i;
        if (!params.year)
            break; // first 10-K is most recent if no year filter
    }
    if (targetIdx === -1) {
...
export async function get10QQuarterlyReport(params) {
    ...
    let targetIdx = -1;
    for (let i = 0; i < recent.form.length; i++) {
        if (!recent.form[i].startsWith('10-Q'))
            continue;
        if (params.year && !recent.filingDate[i].startsWith(String(params.year)))
            continue;
        targetIdx = i;
        if (!params.year)
            break;
...
export async function get8KMaterialEvents(params) {
...
export async function handleTool(toolName, args) {
    switch (toolName) {
        case 'search_company_filings':
            return searchCompanyFilings(args);
        case 'get_10k_annual_report':
            return get10KAnnualReport(args);
        case 'get_10q_quarterly_report':
            return get10QQuarterlyReport(args);
        case 'get_8k_material_events':
            return get8KMaterialEvents(args);
        case 'get_insider_trades':
            return getInsiderTrades(args);
        case 'get_company_info':
            return getCompanyInfo(args);
        default:

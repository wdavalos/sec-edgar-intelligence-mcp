 ...
 */
export async function resolveCIK(companyName: string): Promise<string> {
    ...
    const data = await resp.json() as { results?: Array<{ cik: number; name: string }> };

    if (!data.results || data.results.length === 0) {
        throw new Error(`Company not found on SEC EDGAR: "${companyName}". Try exact name with Inc/Ltd/Corp suffix.`);
    }

    // Exact match preferred, else first result
    const exact = data.results.find(
        (r) => r.name.toUpperCase() === companyName.toUpperCase() ||
               r.name.toUpperCase().includes(companyName.toUpperCase())
    );
    const cikNum = exact ? exact.cik : data.results[0].cik;
...
export async function getCompanySubmissions(cik: string): Promise<CompanySubmissions> {
    ...
    const resp = await throttledFetch(url);
    return resp.json() as Promise<CompanySubmissions>;
}
 ...
 */
export function extractTicker(submissions: CompanySubmissions): string {
 ...
 */
export async function getXBRLFacts(cik: string): Promise<XBRLCompanyFacts> {
    const cikPadded = cik.padStart(10, '0');
 ...
 */
export function buildFilingList(
    submissions: CompanySubmissions,
    opts: { formType?: string; dateFrom?: string; dateTo?: string; maxResults?: number }
): FilingDocument[] {
    ...
    for (let i = 0; i < recent.accessionNumber.length; i++) {
        ...
        // Filter by form type
        if (opts.formType && form !== opts.formType) continue;
        ...
        const cikPadded = String(submissions.cik).padStart(10, '0');
...
export function buildMaterialEvents(
    submissions: CompanySubmissions,
    maxResults: number
): MaterialEvent[] {
    const recent = submissions.filings.recent;
...
export function buildInsiderTrades(
    submissions: CompanySubmissions,
    maxResults: number
): InsiderTrade[] {
    const recent = submissions.filings.recent;
    ...
    for (let i = 0; i < recent.accessionNumber.length; i++) {
        if (!recent.form[i].startsWith('4')) continue;
 ...
 */
export async function get10KContent(cik: string, accessionNumber: string, primaryDocument: string): Promise<{
    ...
    try {
        ...
        const businessDescription = businessDescMatch
            ? businessDescMatch[1].replace(/<[^>]+>/g, '').trim().substring(0, 500)
        ...
        const risk_factors = riskMatch
            ? [riskMatch[1].replace(/<[^>]+>/g, '').trim().substring(0, 500)]
